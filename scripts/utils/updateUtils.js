...
async function updateModuleDataModels() {
    ...
    for (let actor of actors) {
        let actorDataModelVersion = ManaFlagUtils.getManaActorFlag(actor._id, ManaBaseModule.FLAGS.MODULE_VERSION);
...
async function updateV01xToV020(actor) {
    ManaBaseModule.log(true, "Updating actor " + actor.name + " from v0.1.x to v0.2.0");
    ...
    const actorClass = ActorManaFlagUtils.findOriginalClassIdentifier(actorId);
    ...
    const newAttributes = new ManaAttributes(manaCap, manaX, overchargeCap, manaRegen, manaControlDice);
    ManaFlagUtils.setManaActorFlag(actorId, "attributes", newAttributes);
    
    // Step 2
    ManaFlagUtils.setManaActorFlag(actorId, "state", actor.flags["mana-base"].state);
    ...
    ManaFlagUtils.setManaActorFlag(actorId, "_dependent-attributes", dependentAttributes);
    ManaFlagUtils.unsetActorFlag(actorId, "mana-base", "_dependency-attributes");
    
    // Step 4
    const oldExpandedUiFlag = ManaFlagUtils.getManaActorFlag(actorId, "_extended-UI") ? 
        ManaFlagUtils.getManaActorFlag(actorId, "_extended-UI")  : false;
